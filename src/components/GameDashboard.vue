    ...
    <div class="section section-grid">
      ...
      <div class="selection">
        <button
          ...
          :key="option.value"
          @click="updateGridSize(option.value)"
        >
          {{ option.label }}
        </button>
      </div>
    ...
    <div class="section">
      ...
      <div class="selection">
        <button
          ...
          class="button"
          :class="{ 'button-active': gameConfig.numberOfObstacles === option.value }"
          :key="option.value"
          @click="updateNumberOfObstacles(option.value)"
        >
          {{ option.label }}
        </button>
      </div>
    </div>
    <button class="button button-large" @click="$emit('click:newGame', gameConfig)">
      NEW GAME
    </button>
    <div class="author">© 2024 <strong>Prince Sahoo</strong></div>
  </div>
</template>
...
function updateGridSize(gridSize) {
  ...
  emit("updateConfig", gameConfig);
}

function updateNumberOfObstacles(numberOfObstacles) {
  gameConfig.numberOfObstacles = numberOfObstacles;
  emit("updateConfig", gameConfig);
}
</script>
